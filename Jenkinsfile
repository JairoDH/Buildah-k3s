...
'''
    ...
    stages {
        ...
        stage('Migración de base de datos') {
            ...
            steps {
                script {
                    // Realizar la migración de la base de datos desde desarrollo a VPS
                    sshagent(credentials: ['VPS_SSH']) {
                        // Exportar la base de datos desde el entorno de desarrollo
                        sh """
                        ...
                        sh """
                            scp /tmp/backup.sql jairo@fekir.touristmap.es:/tmp/backup.sql
                        """

                        // Importar la base de datos en el VPS
                        sh """
                            ssh jairo@fekir.touristmap.es 'mysql -u${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB} < /tmp/backup.sql'
                        """
                    }
                }
            }
